...
jobs:
  sayHelloWorld:
    ...
    steps:
      - uses: gr2m/hello-world-action@v1.x
```
...
`gr2m/hello-world-action` does the following
...
**Bonus**: This action is releasing automatically to GitHub using [`semantic-release`](https://github.com/semantic-release). It also pushes updates to the `v1.x` branch, which you can reliably depend on in your GitHub workflow (`uses: gr2m/hello-world-action@v1.x`). If there should ever be a breaking release, I'll create a `v2.x` branch, etc.
